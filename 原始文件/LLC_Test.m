...
nRounds = 30;                       % number of random test on the dataset
Ac_num=200;
...
img_dir = '150train';       % directory for the image database                             
data_dir = 'sobel_traindata';       % directory for saving SIFT descriptors
fea_dir = 'Hogtestfeatures';    % directory for saving final image features
dataset='Caltech101';
...
%[database,f] = CalculateSiftDescriptor(rt_img_dir, data_dir, gridSpacing, patchSize, maxImSize, nrml_threshold);
%fpath = database.path{1};
%load(fpath);
[database, lenStat] = calculateEdge(rt_img_dir, data_dir);
% -------------------------------------------------------------------------
